...
: // RNN for the target size (3 * hidden_dim) or 2*hidden_dim
  ...
  builder_src_fwd(1, hidden_dim, hidden_dim, model), // Call the constructor of lstm/gru
...
void AttentionalModel<Builder>::start_new_instance(const std::vector<int> &source, ComputationGraph &cg, const std::vector<int> *ctx)
{
	...
	for (unsigned i = 0; i < slen; ++i){
		Expression temp = lookup(cg, p_cs, source[i]);
...
void AttentionalModel<Builder>::translation_output(Corpus &data, int size, Dict &sd, Dict& td){
	...
	for (unsigned i = 0; i < size; ++i) {
		...
		if (beam > 0)
			target = beam_decode(ssent, cg, beam, td,  nullptr);
		else
			target = greedy_decode(ssent, cg, td, nullptr);
