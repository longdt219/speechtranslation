#include "attentional_mfcc_bound.h"
#include <glob.h>
#include <vector>
...
void generate_alignment(const std::vector<vector<float>> &source, const std::vector<int>& target,
		const std::vector<int>& boundary, ComputationGraph &cg, const Expression &alignment, int docid, int smooth);
...
tuple<Corpus,Corpus> read_corpus_from_vars(std::map<int,string>* eng_sents);
...
int main_body(variables_map vm)
{
	...
	if (vm.count("boundary")){
		// Read the boundary
		cerr<< "Read the boundary directory \n";
		read_boundary_directory(vm["boundary"].as<string>());
	}
	tie(training,devel) = read_corpus_from_vars(&eng_sents);
	...
	for (int j = 0; j < J; ++j) {
		...
		if (max_idx != -1){
			if ((pyramid) && (source.size() > 32))
				...
				if (boundary.size() > 0){
					if (j > 0)
						// Todo : check this, always skip the first one
						out << td.Convert(target[max_idx]) << " " << boundary[j-1] * 10 << " " << boundary[j] * 10 <<  "\n";
				}
				else
...
//		unsigned dev_every_i_reports = 5;
	...
	if (display) {
		...
		for (unsigned i = 0; i < devel.size(); ++i) {
			...
			am.BuildGraph(ssent, tsent, boundary, cg, &alignment, (doco) ? context(devel, i) : nullptr);
			cg.forward();

			cout << "\n====== SENTENCE " << i << " =========\n";
			am.display_ascii(ssent, tsent, boundary, cg, alignment, td);
			cout << "\n";
	...
	if (is_only_alignment){
		...
		for (unsigned i = 0; i < training.size(); ++i) {
			...
			generate_alignment(ssent,tsent,boundary, cg,alignment,docid, smooth_window);
		}
		return;
	}
	...
	if (true) {
		...
		for (unsigned i = 0; i < devel.size(); ++i) {
			tie(ssent, tsent, boundary, docid) = devel[i];
			ComputationGraph cg;
			am.BuildGraph(ssent, tsent, boundary, cg, nullptr, (doco) ? context(devel, i) : nullptr);
...
//			cerr << "Buid graph for this instance " << docid << "\n";
			...
			if (coverage > 0) {
				...
				Expression xent = am.BuildGraph(ssent, tsent, boundary, cg, &alignment, (doco) ? context(training, order[si % order.size()]) : nullptr, &coverage_penalty);
				Expression objective = xent + coverage * coverage_penalty;
				loss += as_scalar(cg.forward());
				penalty += as_scalar(cg.get_value(coverage_penalty.i));
			} else {
				am.BuildGraph(ssent, tsent, boundary, cg, &alignment, (doco) ? context(training, order[si % order.size()]) : nullptr);
		...
		if ((sgd.epoch == max_epochs) || (report % dev_every_i_reports == 0)) {
			...
			for (unsigned i = 0; i < devel.size(); ++i) {
				tie(ssent, tsent, boundary, docid) = devel[i];
				//            		tie(ssent, tsent, docid) = devel[i];
//				cerr << "Report on dev dataset for docID = " << docid << "\n";
				ComputationGraph cg;
				am.BuildGraph(ssent, tsent, boundary, cg, nullptr, (doco) ? context(devel, i) : nullptr);
				dloss += as_scalar(cg.forward());
				dchars += tsent.size() - 1;
...
void read_boundary_directory(const string& foldername){
	...
	for (unsigned int i=0; i<files.size(); ++i){
		...
		while (getline(in,line)){
			...
			int previous = -1;
			for (int i=0; i<substrs.size(); ++i){
				// Must divide to 80 here
				int v = atoi(substrs[i].c_str()) / 80;
				if (v > previous){
					previous = v;
					if (v < mfcc_map[file_idx].size()) // Only pick index that less than the mfcc size
						this_line.push_back(v);
				}
			}
...
tuple<Corpus,Corpus>  read_corpus_from_vars(std::map<int,string>* eng_sents)
{
	...
	for (std::map<int,string>::iterator iter = eng_sents->begin(); iter != eng_sents->end(); ++iter){
		...
		if (mfcc_map.count(key_idx)){
			...
			if (eng_sents->size() - count <= SIZE_DEV){
				develop.push_back(SentencePair(source_sent,target, boundary, key_idx));
			}
			else
				training.push_back(SentencePair(source_sent,target, boundary, key_idx));
	...
	cerr << "Pick " << SIZE_DEV << " sentence from train as development (also use for showing alignment) \n";
