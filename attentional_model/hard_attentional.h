...
struct HardAttentionalModel {
    explicit HardAttentionalModel(Model& model, 
    ...
    Expression BuildGraph(const std::vector<int> &source, const std::vector<int>& target, 
	    unsigned num_samples, ComputationGraph& cg, Expression *alignment=0);
    ...
    Expression add_alignment(int t, ComputationGraph &cg);
    Expression add_target(int tgt_tok, Expression &align, ComputationGraph &cg);
    Expression add_target(int tgt_tok, unsigned j, ComputationGraph &cg);
    Expression add_input(int tgt_tok, int t, ComputationGraph &cg) {
	Expression align = add_alignment(t, cg);
	return add_target(tgt_tok, align, cg);
    }
...
template <class Builder>
HardAttentionalModel<Builder>::HardAttentionalModel(cnn::Model& model,
    unsigned vocab_size_src, unsigned vocab_size_tgt, unsigned layers, unsigned hidden_dim, 
    unsigned align_dim, bool _rnn_src_embeddings, bool _giza_extentions)
...
template <class Builder>
void HardAttentionalModel<Builder>::start_new_instance(const std::vector<int> &source, ComputationGraph &cg)
{
    //slen = source.size() - 1; 
    slen = source.size(); 
...
template <class Builder>
Expression HardAttentionalModel<Builder>::add_alignment(int t, ComputationGraph &cg)
{
    // alignment input 
	...
	i_wah_rep = concatenate_cols(std::vector<Expression>(slen-1, i_wah));
	//WTF(i_wah_rep);
    }
	...
	auto i_tgt_idx = repeat(cg, slen-1, log(1.0 + t));
	//WTF(i_tgt_idx);
...
template <class Builder>
Expression HardAttentionalModel<Builder>::add_target(int trg_tok, Expression &i_alpha_t, ComputationGraph &cg)
{
    ...
    //WTF(i_r_t);
#else
    Expression i_r_t = affine_transform({i_bias, i_R, y_t_t}); 
#endif
...
template <class Builder>
Expression HardAttentionalModel<Builder>::add_target(int trg_tok, unsigned j, ComputationGraph &cg)
{
    Expression i_c_t = pick(src, j); 
    //WTF(i_c_t);
    // word input
    Expression i_x_t = lookup(cg, p_ct, trg_tok);
    //WTF(i_x_t);
...
inline int sample_categorical(const std::vector<float> &dist) {
    // FIXME: can be done faster using binary search, but this requires caching thresholds
    while (true) {
	double p = rand01();
	unsigned w = 0;
	for (; w < dist.size(); ++w) {
...
template <class Builder>
Expression HardAttentionalModel<Builder>::BuildGraph(const std::vector<int> &source,
        const std::vector<int>& target, unsigned num_samples, ComputationGraph& cg, Expression *alignment) 
{
    ...
    for (unsigned t = 0; t < tlen; ++t) {
        Expression i_alpha_t = add_alignment(t, cg);
	aligns.push_back(i_alpha_t);
	if (num_samples == 0) {
	    Expression i_r_t = add_target(target[t], i_alpha_t, cg);
	    //WTF(i_r_t);
	    Expression i_err = pickneglogsoftmax(i_r_t, target[t+1]);
	    errs.push_back(i_err);
	} else {
	    // Stop! Sample time
	    auto adist = as_vector(cg.incremental_forward());

	    std::vector<int> samples;
	    std::vector<Expression> sample_errs;
	    for (unsigned s=0; s < num_samples; ++s) {
		int j = sample_categorical(adist);
		Expression i_r_t = add_target(target[t], j, cg);
		Expression i_err = pickneglogsoftmax(i_r_t, target[t+1]); 
		// FIXME: is this valid; the sample may have effects on
		// predictions later in the sequence through the hidden state
		samples.push_back(j);
		sample_errs.push_back(i_err);
	    }

	    Expression ws = softmax(concatenate(sample_errs));
	    for (unsigned s=0; s < num_samples; ++s) {
		Expression w = pick(ws, s);
		Expression err = w * (pickneglogsoftmax(i_alpha_t, samples[s]) + sample_errs[s]);
		errs.push_back(err);
	    }
	}
    }
...
template <class Builder>
void 
HardAttentionalModel<Builder>::display(const std::vector<int> &source, const std::vector<int>& target, 
                          ComputationGraph &cg, const Expression &alignment, Dict &sd, Dict &td)
{
    using namespace std;
...
template <class Builder>
std::vector<int>
HardAttentionalModel<Builder>::decode(const std::vector<int> &source, ComputationGraph& cg, int beam_width, cnn::Dict &tdict)
{
...
template <class Builder>
std::vector<int>
HardAttentionalModel<Builder>::sample(const std::vector<int> &source, ComputationGraph& cg, cnn::Dict &tdict)
{
    const int sos_sym = tdict.Convert("<s>");
    const int eos_sym = tdict.Convert("</s>");
