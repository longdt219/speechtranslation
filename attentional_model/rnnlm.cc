...
int main(int argc, char** argv) {
  ...
  if (argc != 3 && argc != 4 && argc != 5) {
    cerr << "Usage: " << argv[0] << " corpus.txt dev.txt [model.params] [--test]\n";
    return 1;
  }
  kSOS = d.Convert("<s>");
  kEOS = d.Convert("</s>");
  ...
  if (argc == 5) {
      assert(strcmp(argv[4], "--test") == 0);
      cerr << "Testing: emitting per-sentence normalised cross entropy" << endl;

      for (auto& sent : dev) {
        ComputationGraph cg;
        lm.BuildLMGraph(sent, cg);
        float dloss = as_scalar(cg.forward());
        float chars = sent.size() - 1;
