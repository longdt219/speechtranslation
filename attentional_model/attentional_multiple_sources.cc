#include "attentional_multiple_sources.h"
#include <glob.h>
#include <vector>
...
int main_body(variables_map vm)
{
	...
	if (is_swap) { // This won't be available
...
// Long Duong: Training for each epoch
	while (sgd.epoch < max_epochs) {
		...
		for (unsigned iter = 0; iter < report_every_i; ++iter) {
			...
			tie(ssent1, ssent2,ssent3, tsent, docid) = training[order[si % order.size()]];
			ComputationGraph cg;
			chars += tsent.size() - 1;
			++si;
			Expression alignment;
			if (coverage > 0) {
				...
				Expression xent = am.BuildGraph(ssent1, ssent2, ssent3, tsent, cg, &alignment, (doco) ? context(training, order[si % order.size()]) : nullptr, &coverage_penalty);
				Expression objective = xent + coverage * coverage_penalty;
				loss += as_scalar(cg.forward());
				penalty += as_scalar(cg.get_value(coverage_penalty.i));
			} else {
				am.BuildGraph(ssent1, ssent2, ssent3, tsent, cg, &alignment, (doco) ? context(training, order[si % order.size()]) : nullptr);
				loss += as_scalar(cg.forward());
			}
...
//			cerr << " DEV SIZE = " << devel.size() << "\n";
			for (unsigned i = 0; i < devel.size(); ++i) {
				tie(ssent1, ssent2, ssent3, tsent, docid) = devel[i];
//				cerr << " Doc ID = " << docid << "\n";
				ComputationGraph cg;
				am.BuildGraph(ssent1, ssent2, ssent3, tsent, cg, nullptr, (doco) ? context(devel, i) : nullptr);
...
//	cerr << "SIZE DEV = " << SIZE_DEV << endl;
	...
	for (std::map<int,string>::iterator iter = eng_sents->begin(); iter != eng_sents->end(); ++iter){
		...
		if (phoneme_map.count(key_idx)){
			...
			std::string sent_pair = "<s> " + source_sent + " </s> ||| <s> " + target_sent + " </s>";
			// FIX THE ABOVE SINCE WE NEED  source1 ||| source 2 ||| source3 ||| target


			Sentence source1, source2, source3, target;
			// TODO: need to fix this one, add to build only
			ReadMultipleSentencePair(sent_pair, &source1, &source2, &source3, &sd, &target, &td);
			if (SIZE_DEV ==0)
				training.push_back(SentencePair(source1, source2, source3,target,key_idx));
			else
				if (eng_sents->size() - count <= SIZE_DEV)
					develop.push_back(SentencePair(source1, source2, source3, target,key_idx));
				else
					training.push_back(SentencePair(source1, source2, source3, target,key_idx));

			stoks += source1.size();
			ttoks += target.size();
			if ((source1.front() != kSRC_SOS && source1.back() != kSRC_EOS) ||
					(target.front() != kTGT_SOS && target.back() != kTGT_EOS)) {
...
Corpus read_corpus(const string &filename, bool doco)
{
	...
	while (getline(in, line)) {
		++lc;
		Sentence source1,source2,source3, target;
		ReadMultipleSentencePair(line, &source1,  &source2, &source3, &sd, &target, &td);

		corpus.push_back(SentencePair(source1, source2, source3, target, lc)); // Rather use the lc as the index
		stoks += source1.size();
		ttoks += target.size();

		if ((source1.front() != kSRC_SOS && source1.back() != kSRC_EOS) ||
			(source2.front() != kSRC_SOS && source2.back() != kSRC_EOS) ||
			(source3.front() != kSRC_SOS && source3.back() != kSRC_EOS) ||
				(target.front() != kTGT_SOS && target.back() != kTGT_EOS)) {
...
{
	if (i > 0) {
		int docid = get<4>(corpus.at(i));
		int prev_docid = get<4>(corpus.at(i-1));
		if (docid == prev_docid)
