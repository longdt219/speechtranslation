...
int main_body(variables_map vm)
{
	...
	AttentionalModel<rnn_t> am(&model, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE,
			LAYERS, HIDDEN_DIM, ALIGN_DIM, bidir, giza, doco, fert);
...
// Original  test_rescore function
//template <class AM_t>
...
void test_rescore(Model &model, AM_t &am, string test_file, bool doco)
{
	...
	int count = 0;
	while(getline(in, line)) {
		if (count % 100 == 0) cerr << count << "..";
		count ++;
		Sentence source, target;
		vector<int> num;
		// LD : Need to get the ID here
		string delimiter = "|||";
...
//		cerr << source.size() << "\t" << target.size() << endl;
		...
		// Output the result
		cout << idx1 << " " << delimiter << " " << idx2 << " " << delimiter << " " << 1/loss << endl;
		...
		bool doco, float coverage, bool display, bool fert)
{
	...
	if (training.size() < 2000) dev_every_i_reports = 5;
