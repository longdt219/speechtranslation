...
typedef std::map<int,float> map_data;
typedef vector<int> Sentence;
//typedef pair<Sentence, Sentence> SentencePair;
typedef std::tuple<Sentence, Sentence, Sentence, Sentence, int> SentencePair; // includes document id (optional)
...
struct AttentionalModel {
    explicit AttentionalModel(Model* model, 
	    ...
	    bool giza_extensions, bool doc_context, int _local,
            LookupParameters* cs=0, LookupParameters *ct=0);

    ~AttentionalModel();

    Expression BuildGraph(const std::vector<int>& source1, const std::vector<int>& source2,
    		const std::vector<int>& source3,
    ...
    void display_csv(const std::vector<int> &source, const std::vector<int>& target,
            ComputationGraph& cg, const Expression& alignment, Dict &sd, Dict &td);

    void translation_output(Corpus &data, int size, Dict& sd, Dict& td);

    std::vector<int> greedy_decode(const std::vector<int> &source, ComputationGraph& cg, 
            Dict &tdict, const std::vector<int>* ctx=0);

    std::vector<int> beam_decode(const std::vector<int> &source1, const std::vector<int> &source2,
    		const std::vector<int> &source3,
    		ComputationGraph& cg,
            int beam_width, Dict &tdict, const std::vector<int>* ctx=0);

    // Get the beam keyword
    std::map<int,float> beam_keyword(const std::vector<int> &source, ComputationGraph& cg, int beam_width,
    ...
    Parameters* p_Ta;
    Builder builder;
    Builder builder_src_fwd1;
    Builder builder_src_fwd2;
    Builder builder_src_fwd3;
    Builder builder_src_bwd1;
    Builder builder_src_bwd2;
    Builder builder_src_bwd3;
    ...
    void start_new_instance(const std::vector<int> &src1, const std::vector<int> &src2, const std::vector<int> &src3,
    		ComputationGraph &cg, const std::vector<int> *ctx=0);
    ...
    Expression softmax_logit(Expression* ex, unsigned& size);
    // Set the dropout
...
#define WTF(expression) \
    std::cout << #expression << " has dimensions " << _cg->nodes[expression.i]->dim << std::endl;
#define KTHXBYE(expression) \
    std::cout << _cg->get_value(expression.i) << std::endl;
...
AttentionalModel<Builder>::AttentionalModel(cnn::Model* model,
    ...
    unsigned align_dim, bool _rnn_src_embeddings, bool _giza_extentions, bool _doc_context, int _local,
    LookupParameters* cs, LookupParameters *ct)
: // RNN for the target size (3 * hidden_dim) or 2*hidden_dim
  builder(layers, (_rnn_src_embeddings) ? (1+3*2) * hidden_dim : 2*hidden_dim, hidden_dim, model),

  // source always using just 1 layer
  builder_src_fwd1(1, hidden_dim, hidden_dim, model), // Call the constructor of lstm/gru
  builder_src_fwd2(1, hidden_dim, hidden_dim, model), // Call the constructor of lstm/gru
  builder_src_fwd3(1, hidden_dim, hidden_dim, model), // Call the constructor of lstm/gru
  builder_src_bwd1(1, hidden_dim, hidden_dim, model),
  builder_src_bwd2(1, hidden_dim, hidden_dim, model),
  builder_src_bwd3(1, hidden_dim, hidden_dim, model),
  ...
  // LD: This is kind of initialize the variable only (not very important I suppose)
{
    ...
    if (rnn_src_embeddings) {
    	...
    	// Because we are having 3 different sources
    	p_Ua = model->add_parameters({align_dim, 3*2*hidden_dim});

        p_Q = model->add_parameters({hidden_dim, 3*2*hidden_dim});
    } else {
    ...
    if (doc_context) {
        if (rnn_src_embeddings) {
            p_S = model->add_parameters({hidden_dim, 3*2*hidden_dim});
        } else {
	...
	if (rnn_src_embeddings)
	    p_Wh0.push_back(model->add_parameters({hidden_dim, 3*2*hidden_dim}));
	else
...
template <class Builder>
void AttentionalModel<Builder>::start_new_instance(const std::vector<int> &source1, const std::vector<int> &source2,
		const std::vector<int> &source3,
		ComputationGraph &cg, const std::vector<int> *ctx)
{
    //slen = source.size() - 1;
	assert(source1.size() == source2.size());
	assert(source2.size() == source3.size());

    slen = source1.size();
    std::vector<Expression> source_embeddings;
    if (!rnn_src_embeddings) {
	for (unsigned s = 0; s < slen; ++s) 
	    source_embeddings.push_back(lookup(cg, p_cs, source1[s]));
    } else {
	...
	builder_src_fwd1.new_graph(cg);
	builder_src_fwd1.start_new_sequence();

	for (unsigned i = 0; i < slen; ++i){
		Expression temp = lookup(cg, p_cs, source1[i]);
		drop_based_on_status(&temp);
		src_fwd_1[i] = builder_src_fwd1.add_input(temp);
	}

	std::vector<Expression> src_fwd_2(slen);
	builder_src_fwd2.new_graph(cg);
	builder_src_fwd2.start_new_sequence();

	for (unsigned i = 0; i < slen; ++i){
		Expression temp = lookup(cg, p_cs, source2[i]);
		drop_based_on_status(&temp);
		src_fwd_2[i] = builder_src_fwd2.add_input(temp);
	}

	std::vector<Expression> src_fwd_3(slen);
	builder_src_fwd3.new_graph(cg);
	builder_src_fwd3.start_new_sequence();

	for (unsigned i = 0; i < slen; ++i){
		Expression temp = lookup(cg, p_cs, source3[i]);
		drop_based_on_status(&temp);
		src_fwd_3[i] = builder_src_fwd3.add_input(temp);
	}



	std::vector<Expression> src_bwd_1(slen);
	builder_src_bwd1.new_graph(cg);
	builder_src_bwd1.start_new_sequence();
	for (int i = slen-1; i >= 0; --i) {
	    // offset by one position to the right, to catch </s> and generally
	    // not duplicate the w_t already captured in src_fwd[t]
		Expression temp = lookup(cg, p_cs, source1[i]);
		drop_based_on_status(&temp);
	    src_bwd_1[i] = builder_src_bwd1.add_input(temp);
	}

	std::vector<Expression> src_bwd_2(slen);
	builder_src_bwd2.new_graph(cg);
	builder_src_bwd2.start_new_sequence();
	for (int i = slen-1; i >= 0; --i) {
	    // offset by one position to the right, to catch </s> and generally
	    // not duplicate the w_t already captured in src_fwd[t]
		Expression temp = lookup(cg, p_cs, source2[i]);
		drop_based_on_status(&temp);
	    src_bwd_2[i] = builder_src_bwd2.add_input(temp);
	}

	std::vector<Expression> src_bwd_3(slen);
	builder_src_bwd3.new_graph(cg);
	builder_src_bwd3.start_new_sequence();
	for (int i = slen-1; i >= 0; --i) {
	    // offset by one position to the right, to catch </s> and generally
	    // not duplicate the w_t already captured in src_fwd[t]
		Expression temp = lookup(cg, p_cs, source3[i]);
		drop_based_on_status(&temp);
	    src_bwd_3[i] = builder_src_bwd3.add_input(temp);
	}


	for (unsigned i = 0; i < slen; ++i) 
		// neat way to combine these 2 to form an Expression
		source_embeddings.push_back(concatenate(std::vector<Expression>({src_fwd_1[i], src_fwd_2[i], src_fwd_3[i], src_bwd_1[i], src_bwd_2[i], src_bwd_3[i]})));
    }
    ...
    i_uax = i_Ua * src; // R(align * 6hid) * R(6hid * source_len) = R(align * source_len)
    ...
    if (doc_context && ctx != 0) { 
        ...
        if (!rnn_src_embeddings) {
            ...
            for (unsigned i = 0; i+1 < context.size(); ++i) 
                ctx_embed[i] = builder_src_fwd1.add_input(lookup(cg, p_cs, context[i]));
        }
...
template <class Builder>
Expression AttentionalModel<Builder>::softmax_logit(Expression* ex, unsigned& size)
{
	// Home brew softmax function using logit instead of exp
	Expression temp = logistic(*ex);
//	WTF(temp);
	Expression z = sum_cols(transpose(temp)); // scalar but have to convert to array size |slen|
//	WTF(z);
	Expression z_dup = transpose(concatenate_cols(std::vector<Expression>(size, z)));
//	WTF(z_dup);
	return cdiv(temp,z_dup);
//	return *ex;
}
...
//	WTF(i_wah);
    ...
    if (local_attention == 1){
    		...
    		width = MIN_WIDTH + (MAX_WIDTH - MIN_WIDTH) * logistic(slen * (transpose(i_Vpw) *  tanh(i_Wpw * i_h_tm1)));
    }
...
//    		WTF(mask_exp);
    		...
    		if (smooth_softmax > 1)
    			i_alpha_t = softmax_logit(&i_e_t_masked, slen);
    		else
    			i_alpha_t = softmax(smooth_softmax * i_e_t_masked);
    }
    else{
    		//std::cerr << "Using smooth_softmax = " << smooth_softmax << "\n";
    		if (smooth_softmax > 1)
    			i_alpha_t = softmax_logit(&i_e_t, slen);
    		else
    ...
    // Remember: src in R(6hid * source_len)
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildGraph(const std::vector<int> &source1,
		const std::vector<int> &source2, const std::vector<int> &source3,
        ...
        const std::vector<int>* ctx, Expression *coverage)
{
	...
	start_new_instance(source1, source2, source3, cg, ctx);
//	cerr << "OK Finish start new instance" << endl;
    std::vector<Expression> errs;
    tlen = target.size() - 1;
    for (unsigned t = 0; t < tlen; ++t) {
//    		std::cerr << "with t = " << t << " out of " << tlen << "\n";
        Expression i_r_t = add_input(target[t], t, cg);
//        cerr << "OK Add input" << endl;
...
template <class Builder>
void AttentionalModel<Builder>::translation_output(Corpus &data, int size, Dict &sd, Dict& td){
    int docid;
    const int sos_sym = td.Convert("<s>");
    const int eos_sym = td.Convert("</s>");

    Sentence ssent1, ssent2, ssent3, tsent;
    if (size > data.size()) size = data.size();

	for (unsigned i = 0; i < size; ++i) {
		tie(ssent1, ssent2, ssent3, tsent, docid) = data[i];
		cerr << "Generate translation for : " << i << "\n";
		ComputationGraph cg;
		std::vector<int> target;
		int beam = 5; // Always use greedy decoder
		if (beam > 0)
			target = beam_decode(ssent1, ssent2, ssent3, cg, beam, td,  nullptr);


		bool first = true;
		cout << "Source phone (first only) \n";
		for (auto &w: ssent1) {
			if (!first) cout << " ";
			cout << sd.Convert(w);
			first = false;
		...
		for (auto &w: target) {
			if (!first) cout << " ";
			if ( (w !=sos_sym) && (w !=eos_sym)) // not outputing <s> and </s>
				cout << td.Convert(w);
			first = false;
		}
		cout << endl;
...
template <class Builder>
void 
AttentionalModel<Builder>::display_csv(const std::vector<int> &source, const std::vector<int>& target,
                          ComputationGraph &cg, const Expression &alignment, Dict &sd, Dict &td)
{
    using namespace std;
    ...
    const Tensor &a = cg.get_value(alignment.i);
    // Print header for source
    cout << ",";
    for (int j = 0; j<J; j++){
    	cout << "\"" << sd.Convert(source[j])<<"\",";
    }
    cout << endl;

    for (int i = 0; i < I; ++i) {
    	cout << "\"" << td.Convert(target[i]) << "\",";
        for (int j = 0; j < J; ++j) {
            float v = TensorTools::AccessElement(a, Dim(j, i));
            cout << v << ",";
        }
...
template <class Builder>
std::vector<int>
AttentionalModel<Builder>::beam_decode(const std::vector<int> &source1, const std::vector<int> &source2,
		const std::vector<int> &source3, ComputationGraph& cg, int beam_width, cnn::Dict &tdict, const std::vector<int>* ctx)
{
    const int sos_sym = tdict.Convert("<s>");
    const int eos_sym = tdict.Convert("</s>");
    ...
    // For phone representation, we don't want to generate sentence that longer than phone
    // Note : we don't want generation longer than 100 words
    for (int steps = 0; completed.size() < beam_width && steps < source1.size() && steps < 100; ++steps) {
...
template <class Builder>
std::map<int,float>
AttentionalModel<Builder>::beam_keyword(const std::vector<int> &source, ComputationGraph& cg, int beam_width,
        cnn::Dict &tdict, const std::vector<int>* ctx)
{
    const int sos_sym = tdict.Convert("<s>");
    const int eos_sym = tdict.Convert("</s>");
    ...
    start_new_instance(source, cg, ctx);
//    cerr << " Finished start new instance \n";
    std::vector<Hypothesis> chart;
    chart.push_back(Hypothesis(builder.state(), sos_sym, 0.0f, aligns));

    std::vector<unsigned> vocab(boost::copy_range<std::vector<unsigned>>(boost::irange(0u, vocab_size_tgt)));
    std::vector<Hypothesis> completed;
//    cerr << " Finished initialized \n";
    for (int steps = 0; completed.size() < beam_width && steps < source.size() && steps < 100; ++steps) {
//    	cerr << "Steps = " << steps << endl;
        std::vector<Hypothesis> new_chart;
...
//    cerr << " Completed the char" << endl;
//    cerr << " Size of completed = " << completed.size() << endl;
    ...
    if (completed.size() >0){
    	...
    	for (int i = 0; i< completed.size(); i++){
    		Hypothesis hypo = completed[i];
    		float cost_hypo = hypo.cost / (1.0 * hypo.target.size());
    		for (int j =0; j< hypo.target.size(); j++){
    			float value = 0;
    			int target_idx = hypo.target[j];
    			if (result.find(target_idx) != result.end()) // if this target word id found in result
    				value = result[target_idx];
    			result[target_idx] = value + cost_hypo;
    		}
    	}
    }
    return result;
