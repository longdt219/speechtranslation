...
struct AttentionalModel {
	...
	Expression BuildGraph(const std::vector<std::vector<float>>& source, const std::vector<int>& target,
			const std::vector<int> &boundary, ComputationGraph& cg, Expression* alignment=0, const std::vector<int>* ctx=0,
			Expression *coverage=0);

	void display_ascii(const std::vector<std::vector<float>> &source, const std::vector<int>& target, const std::vector<int>& boundary,
	...
	void start_new_instance(const std::vector<std::vector<float>> &src, const std::vector<int>& boundary,
...
AttentionalModel<Builder>::AttentionalModel(cnn::Model* model, unsigned _vocab_size_tgt, unsigned layers, unsigned hidden_dim,
		...
		// FIX CODE HERE ^_^
		builder_src_fwd(1, 39, hidden_dim, model), // Call the constructor of lstm/gru
		...
		builder_src_bwd(1, 39, hidden_dim, model),
...
template <class Builder>
void AttentionalModel<Builder>::start_new_instance(const std::vector<std::vector<float>> &source,
		const std::vector<int>& boundary, ComputationGraph &cg, const std::vector<int> *ctx)
{
	//slen = source.size() - 1;
	slen = source.size();
...
//		std::cerr << "Boundary size : (finished 1 fwd, 1bwd)" << boundary.size() << "\n";
		if (boundary.size() > 0) {
			// Use the pre-defined boundary
			std::vector<Expression> src_fwd_1(boundary.size());
			builder_src_fwd_1.new_graph(cg);
			builder_src_fwd_1.start_new_sequence();
			for (unsigned i = 0; i < boundary.size(); ++i){
//				std::cerr << slen << "vs boundary " << boundary[i] << " ";
				Expression temp = src_fwd[boundary[i]];
				drop_based_on_status(&temp);
				src_fwd_1[i] = builder_src_fwd_1.add_input(temp);
			}
//			std::cerr<< "Finished the forward computation\n";
			std::vector<Expression> src_bwd_1(boundary.size());
			builder_src_bwd_1.new_graph(cg);
			builder_src_bwd_1.start_new_sequence();
			for (int i = boundary.size()-1; i >= 0; --i) {
				Expression temp = src_bwd[boundary[i]];
				drop_based_on_status(&temp);
				src_bwd_1[i] = builder_src_bwd_1.add_input(temp);
			}
//			std::cerr<< "Finished the backward computation\n";
			// Consider new src_fwd and new src_bwd
			src_fwd = src_fwd_1;
			src_bwd = src_bwd_1;
		}
		if ((pyramid) && (slen > 32)){
		...
		slen = src_fwd.size();
		assert(src_fwd.size() == src_bwd.size());
//		std::cerr << "New source length " << slen << "\n";
...
Expression AttentionalModel<Builder>::BuildGraph(const std::vector<std::vector<float>> &source,
		const std::vector<int>& target, const std::vector<int>& boundary, ComputationGraph& cg, Expression *alignment,
	...
	start_new_instance(source, boundary, cg, ctx);
//		std::cerr << "Add input\n";
